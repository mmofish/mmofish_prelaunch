...
Then /^I should be added to the mailchimp list$/ do
  ...
  pending #Do I have access to hominid from here ? Can I make a helper method to check if an email is subbed ? 
...
And /^it is not associated with a user account$/ do
  (user = User.where(email: params[:email]).first)==false
end

Then /^I should be prompted to log-in$/ do
  #Work out how to do this
  pending #Possibly just direct to the profile page show controller, think it will prompt for login by default
  
end

Then /^I should be redirected to my profile preferences$/ do
  #Work out how to do this
  pending #this may be handled inately from redirected out of the controller ?
end